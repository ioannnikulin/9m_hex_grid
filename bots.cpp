...
using std::pair;
        ...
        while(!won())
        {
            for (auto i: {direction::w, direction::e, direction::d, direction::x, direction::z, direction::a})
            {
                ...
                if (!can_go(i)) field_model[row+deltas.first][col+deltas.second]=wall;///если в эту сторону шагнуть нельзя, запоминаем, что там стена
                if (can_go(i) and field_model[choice_coord.first][choice_coord.second]==fog)
                //else
                {
                    go(i);//если туда шагнуть можно, шагаем, измеряем расстояние до выхода, запоминаем его, и шагаем обратно
                ...
                {
                    choice=i;//запомнить из перебранных соседних клеток ту, которая ближе к выходу
                    choice_coord.first = row+delta(i).first;
                    choice_coord.second = col+delta(i).second;
                }
                /*if (field_model[row+deltas.first][col+deltas.second]!=fog and field_model[row+deltas.first][col+deltas.second]!=wall)
                {
...
private:
    std::pair<int, int> delta(direction dir)
    {
        ...
        if(dir==a) return {-1,row%2};
        if(dir==z) return {0,row%2};
        if(dir==x) return {1,row%2};
        if(dir==d) return {1,-row%2};
        if(dir==e) return {0,-row%2};
    }
